...
def acessos():
    return MongoClient(os.getenv("MONGO_URI"))[os.getenv("MYSQL_DATABASE")]["acessos"]
...
def completar(resposta):
    if hasattr(g, "acesso_id"):
        acessos().update_one({"_id": g.acesso_id}, {"$set": {"status_code": resposta.status_code}})
    return resposta
...
def analise():
    ...
    if len(perfis) >= 2:
        ...
        for perfil, marca in zip(perfis, previsao):
            if marca == -1:
                bloqueados.add(perfil["_id"])
