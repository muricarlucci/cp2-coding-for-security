...
def buscar():
    nome=request.args.get("nome","");con=db();cur=con.cursor(dictionary=True);cur.execute(f"SELECT * FROM usuarios WHERE nome LIKE '%{nome}%'");dados=cur.fetchall();con.close();return jsonify(dados)
@app.get("/perfil")
def perfil():return f"<h1>Bem-vindo, {request.args.get('u','')}</h1>"
...
def relatorio():
    con=db();cur=con.cursor();cur.execute("SELECT * FROM tabela_inexistente");return jsonify(cur.fetchall())
if __name__=="__main__":preparar();app.run(port=5010,debug=True,use_reloader=False,host="127.0.0.1")
