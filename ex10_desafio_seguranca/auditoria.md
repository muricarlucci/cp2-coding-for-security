...
| Ausência de headers de segurança | A02 Security Misconfiguration | Maior superfície para XSS, MIME sniffing e clickjacking | CSP, `nosniff` e `DENY` em todas as respostas. |
| Ausência de logging e alerting | A09 Security Logging and Alerting Failures | Ataques não deixam evidência investigável | Registro estruturado de cada requisição e erro. |
