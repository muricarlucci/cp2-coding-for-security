...
@app.before_request
def registrar():
    logging.info("requisicao metodo=%s rota=%s", request.method, request.path)
...
def admin():
    return request.headers.get("X-API-Key") == os.getenv("ADMIN_API_KEY")
...
def buscar():
    ...
    cur.execute("SELECT id,nome,email,nivel_acesso FROM usuarios WHERE nome LIKE %s", (f"%{nome}%",))
    dados = cur.fetchall()
    con.close()
...
@app.get("/perfil")
def perfil():
    return f"<h1>Bem-vindo, {html.escape(request.args.get('u', ''), quote=True)}</h1>"
...
if __name__ == "__main__":
    preparar()
    app.run(port=5010, debug=False, host="127.0.0.1")
